package com.haswalk.solver.fvm1d.test;

import org.junit.Test;

import com.haswalk.solver.fvm1d.config.Boundary;
import com.haswalk.solver.fvm1d.config.Config1D;
import com.haswalk.solver.fvm1d.config.Config1DBuilder;
import com.haswalk.solver.fvm1d.config.Material;
import com.haswalk.solver.fvm1d.config.Part;
import com.haswalk.solver.fvm1d.config.support.PiecewiseLoadBC;

public class ConfigTest {

	@Test
	public void test1() {
		Config1D config = new Config1DBuilder()
								.material(1)
									.set(Material.ELASTIC_MODULE, 3.27e10)
									.set(Material.DENSITY, 2490)
									.build()
								.boundary(1, Boundary.PIECEWISE_LOAD_BC)
									.set(PiecewiseLoadBC.TIMES, new double[]{0, 0.01, 0.02})
									.set(PiecewiseLoadBC.VALUES, new double[]{0, 10000, 0})
									.build()
								.part(1)
									.set(Part.MESH_URI, "E:/fvm1d/1/mesh.txt")
									.set(Part.MATERIAL_ID, 1)
									.set(Part.OUTPUT_ID, 1)
									.set("loadID", 1, "point", new double[]{})
									.set(Part.GAUGE_POINTS, new double[][]{{}})
									.build()
								.control()
									.time("endTime", 0.02, "factor", 0.667)
									.build()
								.output(1)
									.save()
										.set("inc", -1)
										.set("items", "node_vel", "node_stress")
										.build()
									.gauge()
										.set("inc", -1)
										.set("items", "node_vel", "node_stress")
										.build()
									.build()
								.build();
								
	}
	
	@Test
	public void testConfig2() {
		Config1D config = new Config1DBuilder()
						.material(1)
							.setDensity(2490)
							.setElasticModule(3.27e10)
							.build()
						.material(2)
							.setDensity(2000)
							.setElasticModule(1.56e10)
							.build()
						.boundary(1)
							
						;
	}
	
	@Test
	public void testJsonConfig() {
		
	}
}
